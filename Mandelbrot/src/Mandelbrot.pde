public class Mandelbrot {
  ...
  public Mandelbrot(double r0, double i0, double r1, double i1, double it) {
    state = new MState();
    ...
    for (double py = 0; py < height; py++) {
      for (double px = 0; px < width; px++) {
        ...
        if (c == state.iterations) {
          ...
          if (state.hsb) {
            //double m = doubleMap(c, 0, state.iterations, 1, 0);
            //m = doubleMap(m*m, 1, 0, 0, state.iterations);
            img.pixels[(int)(px + py * width)] = color((float)(c+colorShift)%256, 220, 255);
          } else {
  ...
  public double iterate(double re, double im, double it) {
    ...
    for (double i = 0; i < it; i++) {
      ...
      //tz = dabs(zre);
      //zre = dabs(zre)*dabs(zre)-dabs(zim)*dabs(zim) - re;
      //zim = 2*tz*dabs(zim) - im;
    ...
    if (state.hsb) {
      for (double py = 0; py < height; py++) {
        for (double px = 0; px < width; px++) {
          ...
          if (saturation(c) == 221) {
            img.pixels[(int)(px + py * width)] = color((float)(hue(c)+colorShift)%256, 220, 255);
          }
        }
      }
    }
